...
1. **Linear combination of the inputs.** We first compute a linear predictor
   ...
   where
   - $w_j^{(1)} = (w_{j1}^{(1)},\dots,w_{jp}^{(1)})^\top \in \mathbb{R}^p$ is the weight vector of the $j$-th hidden unit,
   ...
   The weights measure how strongly each original predictor $X_k$ influences the hidden unit and correspond to the arrows from the input nodes to a given hidden node in the network diagram.

2. **Non-linear activation.** We then apply a non-linear activation function $g:\mathbb{R}\to\mathbb{R}$ to this linear predictor,
   $$ Z_j = g\big(b_j^{(1)} + w_j^{(1)\top} X\big), \quad j = 1,\dots,m. $$
