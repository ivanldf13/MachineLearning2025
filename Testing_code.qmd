...
In the next section, we’ll see how **random forests** address this limitation by injecting randomness into tree construction, reducing correlation between trees and thus improving variance reduction beyond bagging.
...
They all split first on similar variables such as *Thal*, *Ca*, or *Oldpeak*, and their overall shapes are nearly identical. This shows how, despite resampling, the bagged trees remain **strongly correlated** because they repeatedly rely on the same dominant predictors for their early decisions.
...
2.  **Grow a tree** on each bootstrap sample. At each node, select $m$ predictors at random from the total $p$ predictors, and choose the best split among these $m$.
...
The **random forest** (orange line) achieves a consistently lower OOB error rate than **bagging** (blue line).\
...
-   For **regression**, we record the total amount that the **Residual Sum of Squares (RSS)** decreases due to splits involving a given predictor $X_j$, for $j = 1, \dots, p$.
...
Random forests can be understood as an **adaptive or weighted nearest-neighbours method**, where “close” observations are assigned different **weights** depending on how frequently they fall into the same terminal nodes as a new observation.
...
This strong generalisation comes from combining many decision trees trained on bootstrapped samples with random subsets of features — a structure that captures **non-linear dependencies** and **complex feature interactions** across the pixel space.
...
These visual patterns align with how humans recognise digits — the model learns to focus on the structural areas that define each number.
...
In other words, variable importance visualisation bridges the gap between performance and explainability, showing how random forests internally differentiate complex image classes.
