...
We introduce slack variables $\xi_i \ge 0$ for training observations $(x_i, y_i)$ with $y_i \in \{-1, +1\}$ and solve the optimisation problem: $$ \min_{\beta_0, \boldsymbol{\beta}, \boldsymbol{\xi}} \left( \frac{1}{2}\lVert \boldsymbol{\beta} \rVert^2 + C \sum_{i=1}^n \xi_i \right) \quad \text{s.t.} \quad y_i(\beta_0 + \boldsymbol{\beta}^{\top}{x}_i) \ge 1 - \xi_i, \ \xi_i \ge 0. $$

Here, the parameter $C > 0$ controls the trade-off between a wide margin (small $\lVert \boldsymbol{\beta} \rVert$) and tolerance for violations (large $\xi_i$). In other words, The parameter C tells the model how much it should care about violations of the margin (i.e. large $\xi_i$ values). The $\xi_i$ variables don’t set the margin; instead, they measure how much each point violates the margin constraint. $C$ controls tolerance, $\xi_i$ measures actual violations and $beta$ determines the margin width.
...
3.  Form the decision function $\hat{f}({x}) = \mathrm{sign}(\beta_0 + {\beta}^{\top}{x})$, supported by training points with non-zero Lagrange multipliers (the support vectors).
...
We introduce slack variables $\xi_i \ge 0$ for training observations $(x_i, y_i)$ with $y_i \in \{-1, +1\}$ and solve the optimisation problem:
$$ \min_{\beta_0, \boldsymbol{\beta}, \boldsymbol{\xi}} \left( \frac{1}{2}\lVert \boldsymbol{\beta} \rVert^2 + C \sum_{i=1}^n \xi_i \right) \quad \text{s.t.} \quad y_i(\beta_0 + \boldsymbol{\beta}^{\top}x_i) \ge 1 - \xi_i, \ \xi_i \ge 0. $$
...
As seen in @fig-ML9_SVC, the **left-hand plot** shows a *hard-margin* classifier, which separates all points perfectly but produces a narrow margin that is sensitive to outliers.  
The **right-hand plot** illustrates the *soft-margin* classifier: some points are allowed inside or even beyond the margin (those with $\xi_i > 0$), resulting in a wider margin and a more stable decision boundary that generalises better.
...
In the support vector classifier, we used the decision function $$f(x) = \beta_0 + \beta_1 x_1 + \cdots + \beta_p x_p, $$ which defines a **linear hyperplane** in the feature space. Once the optimal parameters are found, this function is used to classify new observations: $$ \hat{G}(x) =
\begin{cases}
+1 & \text{if } f(x) > 0, \\
-1 & \text{if } f(x) < 0.
\end{cases} $$
