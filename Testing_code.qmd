...
-   For a regression problem, [^5] for all $B$ bootstrap training sets we grow a large tree **without pruning** to obtain a high-variance predictive model. We then average all these (possibly overfitted) trees to obtain a bagged model with reduced variance.
...
[^5]: In this context, a regression problem refers to a supervised learning task where the target variable $y$ is continuous. In contrast, a classification problem deals with categorical outputs.
...
In the next section, we see how random forests address this limitation by introducing randomness in feature selection (by randomly choosing subsets of predictors at each split)[^6] which reduces correlation between trees and improves variance reduction beyond bagging.

[^6]: The specific mechanism in random forests where, at each split of a tree, the algorithm randomly selects a subset of predictors (features) from the total available ones, instead of considering all features
...
-   The decision tree algorithm is **greedy**: it chooses at each split the variable that gives the largest reduction in impurity.[^7] Since the datasets are very similar, the trees tend to select the **same predictors for their early splits**.

[^7]:Impurity quantifies how mixed the node is. The decision tree algorithm greedily picks the split that most reduces impurity,  that is, makes the resulting groups more homogeneous. In classification, impurity is measured using the Gini index or entropy, while in regression it’s measured using variance (RSS).
...
2.  For iterations $i=1,2,\dots$:
    ...
    2.  Update the parameters with learning rate $\alpha>0$,[^9] $$\theta^{(i)}=\theta^{(i-1)}-\alpha\nabla_\theta J(\theta^{(i-1)}).$$
...
[^9]: $\alpha$ is the learning rate (step size) in gradient descent: it controls how far we move in the direction of the negative gradient at each update. A large $\alpha$ gives big, potentially unstable steps, while a small $\alpha$ yields slow but more stable progress.
