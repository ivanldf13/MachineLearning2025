...
In the next section, we see how random forests address this limitation by introducing randomness in feature selection (by randomly choosing subsets of predictors at each split)[^2] which reduces correlation between trees and improves variance reduction beyond bagging.

[^2]: The specific mechanism in random forests where, at each split of a tree, the algorithm randomly selects a subset of predictors (features) from the total available ones, instead of considering all features
...
-   The decision tree algorithm is **greedy**: it chooses at each split the variable that gives the largest reduction in impurity.[^3] Since the datasets are very similar, the trees tend to select the **same predictors for their early splits**.

[^3]: Impurity quantifies how mixed the node is. The decision tree algorithm greedily picks the split that most reduces impurity, that is, makes the resulting groups more homogeneous. In classification, impurity is measured using the Gini index or entropy, while in regression it’s measured using variance (RSS).
...
\caption{Confusion matrices for bagging and random forest.}
\label{fig-heart-cm}
\end{figure}
\end{minipage}
...
![Variable importance in the heart disease dataset](Images_qmd_ML/ML7_var_imp.png){#fig-var-imp width="50%" fig-align="center"}
...
As seen in @fig-digit-class, the **digit classification** task involves recognising handwritten digits (0–9) based on pixel intensity values. Each model is trained on the same dataset and evaluated on its ability to correctly classify unseen digits.
...
This strong generalisation comes from combining many decision trees trained on bootstrapped samples with random feature subsets, creating a model structure that captures **non-linear dependencies** and **complex feature interactions** across the pixel space.

Regularised logistic models (ridge and lasso) also perform well, as they mitigate overfitting by penalising large coefficients.\
However, random forests remain the most effective approach in this setting, as they can model highly complex decision boundaries while maintaining robustness through averaging and tree decorrelation.
...
We introduce slack variables $\xi_i \ge 0$ for training observations $(x_i, y_i)$ with $y_i \in \{-1, +1\}$ and solve the optimisation problem: $$\min_{\beta_0, \boldsymbol{\beta}, \boldsymbol{\xi}}
\left( \frac{1}{2}\lVert \boldsymbol{\beta} \rVert^2 + C \sum_{i=1}^n \xi_i \right)
\quad \text{s.t.} \quad
y_i(\beta_0 + \boldsymbol{\beta}^{\top}x_i) \ge 1 - \xi_i,\ \xi_i \ge 0.$$

Here, the parameter $C > 0$ controls the trade-off between a wide margin (small $\lVert \boldsymbol{\beta} \rVert$) and tolerance for violations (large $\xi_i$). In other words, $C$ determines how strongly the model penalises margin violations (large $\xi_i$ values). The variables $\xi_i$ **do not set the margin**; instead, they **measure how much each observation violates the margin constraint**.

**Key roles (don’t mix these up):**

-   $\boldsymbol{\beta}$ determines the *margin width* (via $\lVert\boldsymbol{\beta}\rVert$):
    -   A small $\boldsymbol{\beta}$ produces a wider margin, resulting in a support vector classifier with high bias and low variance. The decision boundary is smoother and less sensitive to outliers, making it more stable but potentially unable to capture the true class boundary accurately, leading to underfitting.
    -   A large $\boldsymbol{\beta}$ yields a narrow margin, yielding a classifier with low bias and high variance. The decision boundary fits the training data more closely and can represent the class separation more precisely, but it becomes highly sensitive to individual observations, increasing the risk of overfitting.
-   $\xi_i$ measures the *actual violation* for point $i$.
-   $C$ controls *how costly* violations are (a **penalty** parameter).
-   $b$ (see @sec-tuning-b) controls *how many* violations are allowed (a **budget** parameter).
...
As $C \to \infty$, we recover a hard margin (when separable). A smaller $C$ widens the margin and typically yields a smoother, more stable decision rule focused on the most informative points, i.e., the *support vectors*.
...
Interpretation: the classifier allows some margin violations to gain robustness to outliers, achieve better average classification, and admit solutions when the classes are not perfectly separable.

![Hard- versus soft-margin solutions on the same dataset.](Images_qmd_ML/ML9_SVC.png){#fig-ML9_SVC width="80%"}

As seen in @fig-ML9_SVC, the **left-hand plot** shows the *maximal margin classifier*, which fits the training data perfectly but results in a narrow margin that is highly sensitive to outliers. Even one mislabelled point could completely change the orientation of the separating hyperplane. The **right-hand plot** depicts the *support vector classifier*, where some points are allowed to lie inside or even beyond the margin (those with $\xi_i > 0$). This flexibility leads to a wider margin and a more stable decision boundary.
...
## Mathematical formulation of the support vector classifier (budget form)

The soft-margin classifier can be written as a *margin maximisation* problem with a total violation *budget* $b$. We maximise the margin $M$ while allowing controlled violations via slack variables $\xi_i \ge 0$.
...
**Interpretation of** $\xi_i$ (margin violation for observation $i$):

-   $\xi_i = 0$: on or outside the margin (correctly classified).
-   $0 < \xi_i \le 1$: inside the margin but still correctly classified.
-   $\xi_i > 1$: misclassified (wrong side of the decision boundary).

**Role of** $b$: $b>0$ is the total “budget” of margin violations across all observations.

-   Larger $b$: more total violation allowed (more flexibility).
-   Smaller $b$: less total violation allowed (stricter separation).

**Relationship between** $C$ and $b$:

-   Not the same parameter: $C$ is a **penalty** weight; $b$ is a **budget constraint**.
-   Inversely related in effect:
    -   Large $C$ (violations expensive) $\leftrightarrow$ small $b$ (small violation budget).

    -   Small $C$ (violations cheap) $\leftrightarrow$ large $b$ (large violation budget).

**Support vectors** are points on the margin or violating it (typically $\xi_i>0$); they determine the separating hyperplane.

## Tuning parameter $b$ {#sec-tuning-b}

The parameter $b$ controls flexibility by setting the total violation budget, affecting the number of support vectors and model complexity:

-   **Large** $b$: more violations allowed\
    $\rightarrow$ typically more support vectors and a more flexible boundary\
    $\rightarrow$ **lower bias, higher variance** (overfitting risk increases).
-   **Small** $b$: fewer violations allowed\
    $\rightarrow$ typically fewer support vectors and a simpler boundary\
    $\rightarrow$ **higher bias, lower variance** (underfitting risk increases).

$b$ is typically chosen via cross-validation.

Allowing many margin violations reduces bias because the classifier is not forced to maintain a rigid separation, but it increases variance because the decision boundary becomes sensitive to many support vectors.
...
The support vector classifier produces a *linear* decision boundary in the given feature space. If the true boundary is non-linear, a linear SVC may perform poorly. One can enlarge the feature space (e.g., polynomial features), but this may be computationally expensive; **kernels** address this by creating non-linear decision boundaries implicitly.
...
![Illustration of the kernel trick: linear kernel case and conceptual extension to non-linear kernels.](Images_qmd_ML/ML9_kernel_trick.png){#fig-ML9_kernel_trick fig-align="center" width="70%"}

As seen in @fig-ML9_kernel_trick, the figure illustrates the **linear kernel case**, showing a linear decision boundary with margins in the original predictor space.\
The **kernel trick** consists in replacing the inner product by a kernel function, which implicitly enlarges the feature space so that a **linear boundary in that space** can correspond to a **non-linear boundary** in the original predictor space.

The kernel trick replaces inner products by kernel functions, which implicitly map the data into a higher-dimensional feature space where the SVM fits a linear hyperplane; this corresponds to a non-linear decision boundary in the original space.

**The kernel trick lets a linear classifier behave non-linearly without ever computing non-linear features**
...
The parameter $\gamma$ controls the **scale** of the transformation and hence the flexibility of the decision boundary, in other words, how strongly higher-order polynomial terms matter relative to lower-order ones.
...
In this final example, an SVM with a **radial basis function (RBF) kernel** is tuned automatically using **cross-validation**. Instead of choosing the parameters $\gamma$ and $C$ manually, a grid search is performed over a predefined range of values, and the model is evaluated with $k$-fold cross-validation to identify the best-performing combination.
...
We now return to the **digit classification** problem introduced earlier (see @fig-ML6-digit-class). Support Vector Machines are applied here to classify handwritten digits using a **One-Versus-One** scheme, where one binary classifier is trained for each pair of digits.\
...
The training data consist of pairs $(x_1, y_1), \ldots, (x_n, y_n)$ with $n = 7290$, and are used for model fitting and parameter tuning. Each predictor vector $x_i \in \mathbb{R}^p$ contains the greyscale pixel intensities of an image, and the response variable $y_i \in \{0,1,\ldots,9\}$ indicates the true handwritten digit.

The test data consist of $(\hat{x}_1, \hat{y}_1), \ldots, (\hat{x}_m, \hat{y}_m)$ with $m = 2006$, and are used exclusively to evaluate predictive performance.
...
This is exactly the same softmax function used in multinomial logistic regression. With ReLU in the hidden layer and softmax in the output layer, the single hidden layer neural network directly models the posterior probabilities $$ \hat{f}_i(X) = \Pr(Y = i \mid X) = \text{softmax}\!\big(b^{(2)} + W^{(2)} \max\{0,\, b^{(1)} + W^{(1)} X\}\big)_i, \quad i = 1,\dots,q. $$

For a new input $x_0 \in \mathbb{R}^p$ we then apply the Bayes classifier by predicting the class with the largest estimated probability, $$ \hat{G}(x_0) = \hat{y}_0 = \arg\max_{i=1,\dots,q} \hat{f}_i(x_0). $$
...
Neural networks are non-linear parametric models. All weight matrices and bias vectors that transform one layer into the next are collected into a single parameter vector $$\theta=(W^{(1)},\dots,W^{(M+1)},b^{(1)},\dots,b^{(M+1)}).$$ For a fixed $\theta$ the network defines a predictive model; in regression we denote it by $\hat{f}=\hat{f}_\theta$, and in classification with $q$ classes by $\hat{f}_1,\dots,\hat{f}_q$, where $\hat{f}_j(x)$ is the predicted probability of class $j$.

Given training data $(x_1,y_1),\dots,(x_n,y_n)$, we define a loss function $J(\theta)$ that measures how well the network predicts all responses on the training set. Changing $\theta$ changes the predictions $\hat{f}_\theta(x_i)$ (or $\hat{f}_j(x_i)$) and therefore changes the value of $J(\theta)$.

For regression, where $y_i\in\mathbb{R}$, we use the squared-error loss and define $$J(\theta)=\sum_{i=1}^n\big(y_i-\hat{f}_\theta(x_i)\big)^2.$$

For classification with $q$ classes, $y_i\in\{1,\dots,q\}$, we use the cross-entropy (deviance) loss $$J(\theta)=-\sum_{i=1}^n\sum_{j=1}^q I\{y_i=j\}\log\hat{f}_j(x_i),$$ which is the negative log-likelihood of the multinomial model with class probabilities given by the network outputs $\hat{f}_1(x),\dots,\hat{f}_q(x)$.
...
2.  For iterations $i=1,2,\dots$:
    ...
    2.  Update the parameters with learning rate $\alpha>0$,[^5] $$\theta^{(i)}=\theta^{(i-1)}-\alpha\nabla_\theta J(\theta^{(i-1)}).$$
...
[^5]: $\alpha$ is the learning rate (step size) in gradient descent: it controls how far we move in the direction of the negative gradient at each update. A large $\alpha$ gives big, potentially unstable steps, while a small $\alpha$ yields slow but more stable progress.
...
In the sequel we focus on **regression**; the classification case is entirely analogous but uses the cross-entropy loss instead of the squared error. For regression with squared loss, the gradient of the total loss at some parameter vector $\theta$ can be written as a sum over the training samples, $$\nabla_\theta J(\theta)=\sum_{i=1}^n \nabla_\theta\big(f_\theta(x_i)-y_i\big)^2.$$ This expression says that the overall direction in which we should move $\theta$ is obtained by adding up the contributions of each individual training example.

Because the gradient is a sum, we can look at a **single** training pair $(x,y)$ and study its loss contribution $$L(\theta)=\big(f_\theta(x)-y\big)^2.$$ If we understand how to differentiate this one-sample loss with respect to all parameters, then the full gradient is just the sum of these single-sample gradients. This is exactly the viewpoint that underlies back-propagation and, later, stochastic and mini-batch gradient descent.
...
\cdot
\left.\frac{\partial g(x)}{\partial x}\right|_{x=x_0}.$$ In short-hand notation, if we set $z=g(x)$, this becomes $$\frac{\partial h}{\partial x}=\frac{\partial h}{\partial z}\cdot\frac{\partial z}{\partial x}.$$
...
The forward pass takes a fixed parameter vector $\theta$ and a single training pair $(x,y)$, sends $x$ from the input layer through all hidden layers to the output, computes the prediction $\hat{y}$ and its loss against $y$, and stores all intermediate activations. These stored activations are exactly what back-propagation will use later to compute gradients efficiently.

To compute the gradient for a given parameter vector $\theta$ it is convenient to look at one training pair $(x,y)$ at a time. We fix a single input vector $x = (x_1,\dots,x_p)$ and its target $y$, together with the current weights and biases collected in $\theta$. For this fixed $\theta$ we first run a **forward pass** through the network in order to compute the activation value of every hidden neuron and the final prediction.
...
$$z=g\big(b^{(1)}+W^{(1)}x\big),$$ where $W^{(1)}$ and $b^{(1)}$ are the weights and biases of the first layer and $z$ collects the hidden activations. The output neuron then combines these hidden activations linearly to form the prediction $$\hat{y}=f_\theta(x)=b^{(2)}+w^{(2)\top}z.$$

Conceptually, the forward pass in @fig-backprop-forward takes the fixed input $x$ (green nodes on the left), pushes it through the hidden layer (blue nodes $z_1,z_2$), and produces the output $\hat{y}$ (red node). We then compare $\hat{y}$ with the true target $y$ (green node on the right) and evaluate the loss $$L(\theta)=(\hat{y}-y)^2.$$ In deeper networks the same idea applies: we repeatedly apply “linear transformation + non-linearity” from layer to layer until we reach the output. All these intermediate quantities (the activations in each layer and the prediction) will then be reused in the backward pass to compute the gradients efficiently via the chain rule.
...
From the forward pass we know that, for a single hidden layer, $$z=g\big(b^{(1)}+W^{(1)}x\big),\qquad \hat{y}=f_\theta(x)=b^{(2)}+w^{(2)\top}z,\qquad L(\theta)=(\hat{y}-y)^2.$$ Here $z$ contains the hidden activations, $\hat{y}$ is the prediction for the fixed input $x$, and $L(\theta)$ is the squared-error loss for this one training pair, exactly as in @fig-backprop-forward.
...
Back-propagation applies the **chain rule**: $$\frac{\partial L}{\partial w^{(2)}_1}
=\frac{\partial L}{\partial \hat{y}}\cdot\frac{\partial \hat{y}}{\partial w^{(2)}_1}.$$ For the squared loss $L(\theta)=(\hat{y}-y)^2$ we have $$\frac{\partial L}{\partial\hat{y}}=2(\hat{y}-y),$$ which is just (twice) the prediction error. Since the output is linear in the hidden activations, $$\hat{y}=b^{(2)}+w^{(2)\top}z=b^{(2)}+w^{(2)}_1z_1+w^{(2)}_2z_2,$$ we obtain $$\frac{\partial\hat{y}}{\partial w^{(2)}_1}=z_1.$$ Putting these pieces together gives $$\frac{\partial L}{\partial w^{(2)}_1}=2(\hat{y}-y)\,z_1.$$ By the same argument, $$\frac{\partial L}{\partial w^{(2)}_2}=2(\hat{y}-y)\,z_2,$$ and in general $$\frac{\partial L}{\partial w^{(2)}_i}=2(\hat{y}-y)\,z_i.$$
...
We now move one step further back and compute derivatives with respect to the **first-layer weights**. Focus first on a single weight $w^{(1)}_{11}$, which connects input $x_1$ to hidden unit $z_1$ in @fig-backprop-forward. Along this path the loss depends on $w^{(1)}_{11}$ via $$w^{(1)}_{11}\;\longrightarrow\;z_1\;\longrightarrow\;\hat{y}\;\longrightarrow\;L,$$ so the chain rule gives $$\frac{\partial L}{\partial w^{(1)}_{11}}
=\frac{\partial L}{\partial \hat{y}}\cdot\frac{\partial \hat{y}}{\partial z_1}\cdot\frac{\partial z_1}{\partial w^{(1)}_{11}}.$$

We already know $\partial L/\partial\hat{y}=2(\hat{y}-y)$. The second factor is the sensitivity of the output to the hidden unit: $$\frac{\partial \hat{y}}{\partial z_1}=w^{(2)}_1,$$ because $\hat{y}=b^{(2)}+w^{(2)}_1z_1+w^{(2)}_2z_2$. For the third factor we use the definition of the hidden activation, $$z_1=g\big(b^{(1)}_1+w^{(1)\top}_1 x\big),$$ where $w^{(1)}_1$ is the row of $W^{(1)}$ corresponding to hidden unit $z_1$. Differentiating with respect to $w^{(1)}_{11}$ yields $$\frac{\partial z_1}{\partial w^{(1)}_{11}}
=g'\big(b^{(1)}_1+w^{(1)\top}_1 x\big)\,x_1.$$

Combining the three factors we obtain the explicit formula from the slide: $$\frac{\partial L}{\partial w^{(1)}_{11}}
=2(\hat{y}-y)\cdot w^{(2)}_1\cdot g'\big(b^{(1)}_1+w^{(1)\top}_1 x\big)\,x_1.$$

Exactly the same logic applies to any first-layer weight $w^{(1)}_{ij}$, which connects input $x_j$ to hidden unit $z_i$. Along the path $$w^{(1)}_{ij}\;\longrightarrow\;z_i\;\longrightarrow\;\hat{y}\;\longrightarrow\;L$$ we obtain $$\frac{\partial L}{\partial w^{(1)}_{ij}}
...
For the output-layer weights we had $$\frac{\partial L}{\partial w^{(2)}_i}=2(\hat{y}-y)\,z_i.$$ We now define a single scalar $$\delta^{(2)}=2(\hat{y}-y),$$ which is the error at the output neuron for the current parameters $\theta$. Then $$\frac{\partial L}{\partial w^{(2)}_i}=\delta^{(2)}z_i.$$

For the first-layer weights we found $$\frac{\partial L}{\partial w^{(1)}_{ij}}
=2(\hat{y}-y)\cdot w^{(2)}_i\cdot g'\big(b^{(1)}_i+w^{(1)\top}_i x\big)\,x_j.$$ We now define, for each hidden unit $i$, $$\delta^{(1)}_i
=2(\hat{y}-y)\cdot w^{(2)}_i\cdot g'\big(b^{(1)}_i+w^{(1)\top}_i x\big),$$ so that $$\frac{\partial L}{\partial w^{(1)}_{ij}}=\delta^{(1)}_i x_j.$$

The quantities $\delta^{(2)}$ and $\delta^{(1)}_i$ are the **errors** associated with the output neuron and the $i$-th hidden neuron, respectively. They satisfy the simple relationship $$\delta^{(1)}_i=\delta^{(2)}\cdot w^{(2)}_i\cdot g'\big(b^{(1)}_i+w^{(1)\top}_i x\big),$$ which is exactly the expression on the slide: the error at a hidden node is the error at the next layer, multiplied by the connecting weight and by the derivative of the activation at that node.

For the ReLU activation $g(x)=\max\{0,x\}$ we have $$g'(x)={1}\{x>0\},$$ so the hidden error $\delta^{(1)}_i$ is either zero (if the neuron is inactive) or proportional to $\delta^{(2)}w^{(2)}_i$ (if the neuron is active).
...
2.  **Backward pass.**
    ...
    -   Finally compute the gradients $$\frac{\partial L}{\partial w^{(2)}_i}=\delta^{(2)}z_i,\qquad
...
Recall from the gradient descent section that the loss for regression is $$J(\theta)=\sum_{i=1}^n\big(y_i-\hat f_\theta(x_i)\big)^2,$$ so its gradient is the sum of the per-sample gradients $$\nabla_\theta J(\theta)=\sum_{i=1}^n\nabla_\theta\big(y_i-\hat f_\theta(x_i)\big)^2.$$ Back-propagation gives us exactly these single-sample gradients: for each training pair $(x_i,y_i)$ we can run a forward pass, compute the loss, and then a backward pass to obtain $\nabla_\theta\big(y_i-\hat f_\theta(x_i)\big)^2$.

To improve the error function $J(\theta)$ we now have two options; the first one is **batch optimisation** (the classical setting):
...
-   We then take a gradient descent step $$\theta\leftarrow\theta-\alpha\,\nabla_\theta J(\theta),$$ where $\alpha>0$ is the learning rate.
...
For regression the loss is $$J(\theta)=\sum_{i=1}^n\big(y_i-\hat f_\theta(x_i)\big)^2,$$ so the exact gradient needed for gradient descent is $$\nabla_\theta J(\theta)=\sum_{i=1}^n\nabla_\theta\big(y_i-\hat f_\theta(x_i)\big)^2.$$ In batch optimisation we use **all** $n$ training samples to compute this gradient before each update. When $n$ is very large (millions or billions of examples), a single full gradient step already becomes prohibitively slow.
...
-   A general recommendation is to initialise all weights as **small random numbers**, for example drawn from a normal or uniform distribution centred at zero.
...
Regularisation explicitly **prevents overfitting**: we sacrifice some training fit in order to improve the **generalisation error** (test error). We have several complementary tools:

### Parameter penalties

As in ridge and lasso regression, we can add a **penalty term** to the loss and minimise the **penalised objective** $$J_{\text{reg}}(\theta)=J(\theta)+\alpha\Omega(\theta),$$ where $\alpha>0$ is a tuning parameter and $\Omega(\theta)$ measures the overall size of the parameters.

A common choice is an $\ell_2$-type penalty on the weights: $$\Omega(\theta)=\sum_{m,i,j}\big(w^{(m)}_{ij}\big)^2,$$ i.e. we sum the squared values of all weights $w^{(m)}_{ij}$ over all layers $m$ and connections $(i,j)$; often biases are excluded or penalised more weakly. This **shrinks the weights towards zero** and makes the network less “wiggly”, in exactly the same spirit as ridge regression. One can also use an $\ell_1$ penalty to encourage sparsity, but in neural nets we typically care less about exact zero coefficients than we did in lasso, because individual weights have no simple interpretation.
...
-   $\alpha$ balances these two forces: large $\alpha$ means strong regularisation (simpler networks, higher bias, lower variance); small $\alpha$ means weak regularisation (very flexible networks, lower bias, higher variance).

As in other regularised models, $\alpha$ is typically chosen by **validation or cross-validation**, looking at the behaviour of the **validation error**, not the training error.

## Dropout

Dropout could be considered as a **completely different** type of regularisation: instead of adding a penalty, we change the **network structure at every gradient step**.
...
-   for hidden units, a typical choice is $q\approx0.5$ (so each hidden neuron is kept with probability $0.5$);
-   for input units, a smaller probability is used, e.g. $q\approx0.2$ (each input feature is kept with probability $0.8$).
...
In @fig-dropout-bagging you see this visually: the grey input and hidden units are dropped for the current step, while the blue units form the “thinned” network that actually propagates the signal from the inputs to the outputs.
...
We could make a clear analogy to **bagging / random forests**: if we look at the input layer, dropout means some predictors (say $X_3$) are simply not used in that step, just like in random forests we exclude some variables in individual trees. Over many gradient steps, we are effectively training **many slightly different networks**, each seeing a different subset of units.

In simple cases one can interpret the final model as a kind of **ensemble average** over all these thinned networks: we combine the fitted weights and the dropout probabilities, just as bagging combines predictions from bootstrap samples. This ensemble view explains why dropout leads to a more **robust final predictor**. @fig-dropout-bagging illustrates this “ensemble of thinned networks” perspective.

![Dropout as an implicit ensemble: across many training steps, different thinned networks are fitted, and the final predictor behaves like an average over these models.](Images_qmd_ML/ML11_regularisation_2.png){#fig-dropout-bagging width="80%" fig-align="center"}
...
Probably the **most commonly used** regularisation method in deep learning is **early stopping**. Instead of modifying the loss function or the network architecture, early stopping intervenes directly in the **optimisation process**.

The key observation is the following:

- For large networks with enough **capacity to overfit**, the **training error** typically decreases steadily as gradient descent progresses.
- At the same time, the **validation error** (which approximates the test error) often decreases only up to a point and then **starts to increase**, indicating overfitting.

Early stopping exploits this behaviour. Rather than running gradient descent until convergence, we:

- monitor the validation error over training iterations (or epochs);
- identify the iteration $i^*$ at which the **validation error is minimal**;
- **stop the optimisation** at that point and return the parameter vector $\theta^{(i^*)}$.

In other words, we deliberately **do not** go all the way to a (local) minimum of the training loss $J(\theta)$. Instead, we stop at a parameter configuration that gives the **best generalisation performance**, even if the training error could still be reduced further.

This makes early stopping a form of **implicit regularisation**: it limits how complex the fitted model becomes by preventing the parameters from fully adapting to noise in the training data.

An interesting theoretical result is that, in some simplified settings, **early stopping can be shown to be equivalent to $\ell_2$ regularisation**. Intuitively, stopping early keeps the weights small, just like an explicit ridge penalty would.

## Parameter sharing

Another important regularisation principle is **parameter sharing**. Here, regularisation is achieved by **reducing the number of free parameters** in the model.

The idea is motivated by **invariances** or **domain knowledge**. In many applications, it makes sense to **force groups of parameters to have the same value**, rather than estimating them independently.

- By sharing parameters, we drastically **reduce the number of unique parameters**.
- This lowers model complexity and therefore acts as a strong form of regularisation.
- At the same time, it encodes useful structural assumptions about the problem into the model.

The most prominent example of parameter sharing appears in **convolutional neural networks (CNNs)** for image recognition. There, the same filter (i.e. the same set of weights) is applied across different spatial locations of the image, reflecting the assumption that relevant patterns can occur anywhere in the image.

Overall, regularisation for deep neural networks is an **active area of research**, and in practice several techniques (parameter penalties, dropout, early stopping, and parameter sharing) are often combined to achieve good generalisation.

# Deep learning: Convolutional Neural Networks

## Image recognition: CIFAR-10

To illustrate convolutional networks, we will work with the CIFAR-10 data set, a standard benchmark for image recognition. It consists of 60 000 colour images of size $(32 \times 32)$ pixels, each belonging to one of 10 object classes: $$ \mathcal{G} = \{\text{airplane}, \text{automobile}, \text{bird}, \text{cat}, \text{deer}, \text{dog}, \text{frog}, \text{horse}, \text{ship}, \text{truck}\}. $$
...
![Examples of CIFAR-10 images](Images_qmd_ML/ML12_imag.png){#fig-cifar10-examples width="60%" fig-align="center"}

## A dense neural network

For the CIFAR-10 data set, each image is represented by $(p = 1024)$ predictors (32 × 32 pixels × 3 colour channels).

A **dense** (fully connected) neural network connects every neuron in one layer to every neuron in the next, each connection having its own weight. If we build a dense network for CIFAR-10 with hidden layers of size 1024, then each dense layer contains roughly $$ 1024 \times 1024 \approx 10^6 $$ weights, plus biases. So each layer has about one million parameters, and the full network has several million.
...
As illustrated in @fig-dense-cifar, this architecture also ignores the spatial structure of images: the pixels are simply stacked into a long vector $X_1,\dots,X_{1024}$, and the network learns separate weights for each connection. This motivates convolutional neural networks, which drastically reduce the number of parameters by sharing weights and by explicitly exploiting locality in images.

![Dense fully connected neural network for CIFAR-10 classification](Images_qmd_ML/ML12_dense.png){#fig-dense-cifar width="80%" fig-align="center"}

## Spatial structures in image data

So far we have treated images as long predictor vectors $X_1,\dots,X_{1024}$ and used generic regularisation (ridge, dropout, etc.) to fight overfitting. For image data, however, we have extra **domain knowledge** we can exploit: there is a natural **spatial structure** in the predictors. Neighbouring pixels in an image tend to look similar, while pixels that are far apart are usually less related.

A very effective way to regularise a neural network is therefore to **decrease the number of parameters in its architecture**, using this spatial structure. Instead of learning completely separate weights for every pixel–neuron pair, we can **share parameters** across locations that “play the same role” in different parts of the image (e.g. all local $3 \times 3$ patches). This reduces model complexity and builds in the prior belief that local patterns (edges, corners, small shapes) repeat across the image.

![CIFAR-10 car example illustrating spatial structure in neighbouring pixels](Images_qmd_ML/ML12_spatial_car.png){#fig-spatial-car width="45%" fig-align="center"}

In the CIFAR-10 car image shown in @fig-spatial-car, for example, neighbouring pixels along the car roof or the road are very similar, whereas a pixel on the sky and a pixel on a tyre are quite different. Convolutional neural networks make this intuition explicit in their architecture: they use local filters that look at small neighbourhoods of pixels and **reuse the same filter weights across the whole image**, achieving parameter sharing and exploiting spatial structure at the same time.
...
The first stage of a convolutional layer is the **convolution operation**. It takes a small local patch of the image, called the **receptive field** (for example a $3 \times 3$ block of pixels), and combines its values linearly into a single score. The weights of this linear combination form a **filter** (or kernel).

In the illustration @fig-conv-stage, the left grid is the input image $I$, the middle grid is a $3 \times 3$ filter $K$, and the right grid is the **convolved image** $I * K$:

-   We place the filter on top of a $3 \times 3$ receptive field in $I$ (red square).
...
Formally, for a receptive field of the same size as $K$, the output at location $(i,j)$ is $$ (I * K)_{ij} = \sum_{u,v} I_{i+u,\,j+v}\,K_{u,v}. $$So convolution is just a dot product between the filter and the local image patch.
...
![Illustration of a 3×3 convolution: a receptive field in the input image I is combined with a filter K to produce one entry of the convolved image I \* K.](Images_qmd_ML/ML12_conv_stage.png){#fig-conv-stage width="70%" fig-align="center"}

## The convolution stage: hyperparameters
...
-   **Filter width and height.**\
    The first choice is the spatial size of the filter (for example $3 \times 3$ or $5 \times 5$). Larger filters see a bigger neighbourhood but also increase the number of parameters and computations.
...
![Convolution hyperparameters: (a) effect of stride on output size; (b) zero-padding to keep input and output sizes the same.](Images_qmd_ML/ML12_conv_hparams.png){#fig-conv-hparams width="100%" fig-align="center"}

## The detector and pooling stages

The **second stage** of the convolutional layer is the **detector stage**. Here we apply a fixed non-linearity, usually the ReLU, $$ \text{ReLU}(t) = \max\{0, t\}, $$ independently to every score produced by the convolution. A neuron is **active** (fires) only if the convolution score at that location is large enough, so a high ReLU output means that the feature encoded by the filter (for example a vertical edge) is present in that part of the image. This stage usually introduces **no additional parameters**, because the form of the non-linearity is chosen in advance.

The **third stage** is **pooling**, a form of **downsampling**. Pooling aggregates nearby activations into a single value, thereby reducing the spatial resolution of the feature map. The most common choice is **max-pooling**: we partition the feature map into small blocks (often $2 \times 2$) and keep only the maximum activation in each block.

In the left part of @fig-pooling, a $4 \times 4$ grid of activations is transformed by $2 \times 2$ max-pooling into a $2 \times 2$ grid: in each coloured $2 \times 2$ block we keep only the largest number (20, 30, 112, 37). This throws away information, but it has two important effects:

-   it **reduces the image size**, so later layers have fewer units and parameters;\
-   it makes the representation more **robust to small translations**: if a feature (say, a vertical edge) moves slightly within a $2 \times 2$ block, the pooled value stays the same, so the network still registers that the feature is present somewhere in that region.

The right part of @fig-pooling shows the same idea in a real CNN: a feature map of size $224 \times 224 \times 64$ is downsampled by pooling to $112 \times 112 \times 64$. Repeating convolution–ReLU–pooling blocks many times lets us gradually shrink the spatial dimensions while keeping only the most important activations, so that later we can afford to add dense layers on a much smaller representation.

![Detector (ReLU) and 2×2 max-pooling: local downsampling of activation maps and reduction from 224×224×64 to 112×112×64.](Images_qmd_ML/ML12_pooling.png){#fig-pooling width="90%" fig-align="center"}

## The convolutional layer

We can now put the three stages together. In this course, a **convolutional layer** means:

convolution operation → ReLU (detector stage) → pooling.
...
In @fig-conv-car we start from a greyscale image of a car. Below, @fig-conv-filters shows four simple $4 \times 4$ filters whose entries are $-1$ (black) or $+1$ (white). They are chosen by hand to behave like oriented **edge detectors**:
...
![Greyscale input image of a car](Images_qmd_ML/ML12_car.png){#fig-conv-car width="40%" fig-align="center"}

![Four 4×4 edge-detecting filters](Images_qmd_ML/ML12_filter.png){#fig-conv-filters width="80%" fig-align="center"}

![Activation maps for the four filters](Images_qmd_ML/ML12_activation.png){#fig-conv-activations width="95%" fig-align="center"}
...
For one feature map, we can write these stages as $$ Z_j^{(1)} = b^{(1)} + w^{(1)\top}_j X, \quad j = 1,\dots,m_1, $$ $$ Z_j^{(2)} = \max\{0, Z_j^{(1)}\}, \quad j = 1,\dots,m_1, $$ $$ Z_j^{(3)} = \text{pool}(Z^{(2)})_j, \quad j = 1,\dots,m_3. $$
...
-   All biases are equal: $b_1^{(1)} = \cdots = b_{m_1}^{(1)} = b^{(1)}$.
...
As illustrated in the 1D toy example in @fig-conv-maths, a single $3 \times 1$ filter slides over an input of six pixels (plus zero-padding). The corresponding convolution stage uses the same three non-zero weights and one shared bias at every location. After ReLU and pooling, only three pooled outputs remain.

A crucial consequence is that the **number of parameters does not depend on the image size or the number of output locations**. For a single filter of height H and width W we only need $$ \text{H} \times \text{W} + 1 $$ parameters (H×W filter weights plus one bias). In the example of @fig-conv-maths, this is $3 \times 1 + 1 = 4$, much smaller than a dense layer that would use a different set of weights at every position.

![1D toy example of a convolutional layer with zero-padding, convolution, ReLU and pooling, illustrating weight sharing and the small number of parameters.](Images_qmd_ML/ML12_maths.png){#fig-conv-maths width="65%" fig-align="center"}
...
If each filter has height H and width W, and we use one shared bias per filter, then the total number of parameters in a convolutional layer is $$(\text{H} \times \text{W} + 1) \times \text{filter depth}.$$ So the parameter cost grows with the filter size and the number of filters, but **not** with the spatial size of the input image. In the first convolutional layer of @fig-typical-cnn we use 10 filters of size $5 \times 5$, each with one bias: $$(5 \times 5 + 1) \times 10 = 260 \text{ parameters.}$$ A dense layer connecting a $28 \times 28$ input directly to, say, 100 hidden units would already need $28 \times 28 \times 100 = 78\,400$ weights, so the saving is substantial.

The architecture in @fig-typical-cnn shows a **typical CNN for MNIST digits**:

-   Input: a $28 \times 28$ greyscale image of a digit.
-   First convolutional layer: 10 filters of size $5 \times 5$ produce 10 feature maps of size $24 \times 24$.
-   Max-pooling layer: $2 \times 2$ pooling reduces these to $12 \times 12 \times 10$.
...
![Example of a typical CNN architecture for MNIST digits, with two convolution–pooling blocks followed by flattening and fully connected layers.](Images_qmd_ML/ML12_typical.png){#fig-typical-cnn width="95%" fig-align="center"}
...
\item \textbf{log-lasso:} logistic regression with lasso penalty.  
\item \textbf{RF:} random forest.  
\item \textbf{linear SVM:} linear support vector classifier.  
\item \textbf{radial SVM:} SVM with radial basis kernel.  
\item \textbf{FC NN:} fully connected neural network.  
...
## Activations and filters of the first layer
...
In @fig-first-layer-acts, each row shows:
...
![Activations and filters of the first convolutional layer: each row shows an input digit, the corresponding activation maps of several filters, and examples of the learned filters themselves.](Images_qmd_ML/ML12_first_layer.png){#fig-first-layer-acts fig-align="center" width="60%"}
...
-   Images have **higher and variable resolution**; the average size is about $482 \times 415$ pixels, so each raw image is on the order of $2 \cdot 10^5$ pixels instead of 1024.
...
Since LeNet, a whole **“zoo” of CNN architectures** has been proposed, each trying to improve benchmark performance on data sets such as CIFAR-10 and ImageNet. Examples include AlexNet, VGG, Inception and ResNet. They mainly differ in how many layers they use, how they arrange convolution and pooling, and how they choose filter sizes, but they all build on the same core principles introduced by LeNet.
...
## Typical filter weights of the first layer

The **filters of the first convolutional layer** can often be visualised directly, since they operate on the raw pixels. As shown in @fig-firstlayer-filters, when we plot these filters for a CNN trained on natural images, they typically look like:
...
![Typical first-layer filters of a CNN trained on natural images. Many resemble oriented edge detectors or small colour blobs.](Images_qmd_ML/ML12_firstlayer_filters.png){#fig-firstlayer-filters width="60%" fig-align="center"}

## Unconverged filters

On the previous slide we saw clean first-layer filters that behaved like edge or blob detectors. In contrast, as shown in @fig-unconverged-filters, sometimes the learned filters look more like noisy static: they do **not** show clear structure and do not extract clear features.
...
![Example of unconverged first-layer filters that look noisy and do not exhibit clear edge or blob patterns.](Images_qmd_ML/ML12_unconverged_filters.png){#fig-unconverged-filters width="50%" fig-align="center"}
...
As shown in @fig-deeper-activations, each row in the visualisation corresponds to one deep neuron. We see, for example:
...
![Images that maximally activate six neurons in a deep pooling layer of AlexNet, showing specialisation to faces, dogs, text, houses and other high-level patterns.](Images_qmd_ML/ML12_deeper_activations.png){#fig-deeper-activations width="95%" fig-align="center"}
...
How does this work in practice? As shown in @fig-transfer-cat, it helps to view a CNN as consisting of two conceptual parts:
...
![Transfer learning: reuse the convolutional feature extractor and retrain a new task-specific classifier on top.](Images_qmd_ML/ML12_cat.png){#fig-transfer-cat width="80%" fig-align="center"}

## Transfer learning: mathematical view

We can describe transfer learning more formally by splitting a CNN into two functions:

-   a **feature extractor** that maps an input image $x$ to a vector of feature activations $$ z = f(x) = (z_1,\dots,z_m), $$
-   a **classifier** that maps the features to output scores or probabilities $$ y = g(z). $$

When a network is trained on a large data set such as ImageNet, we learn parameters $\theta_f$ of the feature extractor and $\theta_g$ of the classifier: $$ z = f_{\theta_f}(x), \qquad y = g_{\theta_g}(z). $$

In **transfer learning** for a new, smaller data set we proceed as follows:

1.  **Start from the ImageNet-trained network.**\
    The convolution + pooling layers define $f_{\theta_f}$; the final fully connected layers define $g_{\theta_g}$.

2.  **Freeze the feature extractor.**\
    We keep $\theta_f$ fixed and reuse it as a generic map that turns any input image into a feature vector $z = f_{\theta_f}(x)$.

3.  **Replace and retrain the classifier.**\
    We discard the original last layer(s), introduce a new classifier $g_{\phi}$ with parameters $\phi$ (for example a softmax over our own classes), and train only $\phi$ on the small task-specific data set: $$ y = g_{\phi}(f_{\theta_f}(x)). $$
...
Mathematically, transfer learning is thus the reuse of a pre-trained feature map $f_{\theta_f}$ and the **replacement of the last part of the network** by a new classifier tailored to the target problem.

## Transfer learning: practical strategy

A simple and widely used transfer-learning strategy is illustrated in @fig-transfer-strategy:
...
![Transfer-learning strategy: use AlexNet (trained on ImageNet) as a fixed feature extractor and train a new linear classifier on the 4096-dimensional codes from the last but one layer.](Images_qmd_ML/ML12_transfer.png){#fig-transfer-strategy width="80%" fig-align="center"}
...
## Choosing a transfer-learning strategy
