...
Support Vector Machines classify observations using a function that is, in general, *non-linear* in the original feature space: $$ f(\mathbf{x}) = \beta_0 + \sum_{i=1}^{n} \alpha_i y_i K(\mathbf{x}, \mathbf{x}_i), $$ where $K(\mathbf{x}, \mathbf{z})$ is a *(positive definite)* **kernel function** measuring the similarity between two feature vectors $\mathbf{x}, \mathbf{z} \in \mathbb{R}^p$.
...
The **polynomial kernel** is defined as $$ K(\mathbf{x}, \mathbf{z}) = \left( 1 + \gamma \sum_{j=1}^{p} x_j z_j \right)^d, $$ where $\gamma$ is a scaling parameter and $d$ is the degree of the polynomial.
...
The **radial basis function (RBF)** or **Gaussian kernel** is defined as $$ K(\mathbf{x}, \mathbf{z}) = \exp \left( -\gamma \sum_{j=1}^{p} (x_j - z_j)^2 \right), $$ where $\gamma > 0$ determines how quickly similarity decays with distance.
...
Although the underlying mathematics are complex, the **kernel trick** allows us to apply SVMs without needing to explicitly compute the high-dimensional feature mappings. In practice, fitting an SVM simply requires computing the pairwise similarities $$ K(\mathbf{x}_i, \mathbf{x}_j) $$ for all distinct pairs of training observations $(i, j)$.
...
\begin{itemize}
\tightlist
\item Training data $(\mathbf{x}_1, y_1), \ldots, (\mathbf{x}_n, y_n)$, $n = 7290$, are used for model fitting and parameter tuning.  
  \begin{itemize}
  \tightlist
  \item Predictors $\mathbf{x}_i$ are greyscale pixel intensities of each image $\in \mathbb{R}^p$.  
  \item Outcomes $y_i$ correspond to the true digit labels $\in \{0, 1, \ldots, 9\}$.
  \end{itemize}
\item Test data $(\hat{\mathbf{x}}_1, \hat{y}_1), \ldots, (\hat{\mathbf{x}}_m, \hat{y}_m)$, $m = 2006$, are used only for evaluating predictive performance.
\end{itemize}
...
We now introduce the simplest neural network architecture for a quantitative response $Y$. As in previous regression methods, we start from $p$ original predictors $X = (X_1,\dots,X_p)^\top$ and would like to build a flexible model for $Y$. The key idea of a single hidden layer neural network is to construct $m$ new intermediate variables, called hidden units or hidden neurons, which play the role of automatically engineered features.

Each hidden unit $Z_j$ is obtained in two steps. First, we form a linear combination of the inputs with its own intercept, $$ Z_j^{\text{lin}} = b_j^{(1)} + w_j^{(1)\top} X, \quad j = 1,\dots,m, $$ where $w_j^{(1)} = (w_{j1}^{(1)},\dots,w_{jp}^{(1)})^\top \in \mathbb{R}^p$ is the weight vector attached to the $j$-th hidden unit and $b_j^{(1)} \in \mathbb{R}$ is its bias (an intercept term inside this inner linear model; it is unrelated to the “bias” from the bias–variance trade-off). The weights measure how strongly each original predictor $X_k$ influences the hidden unit, and correspond to the arrows from the input nodes to a given hidden node in the network diagram.

Second, we apply a non-linear activation function $g:\mathbb{R}\to\mathbb{R}$ to this linear combination, which yields $$ Z_j = g\big(b_j^{(1)} + w_j^{(1)\top} X\big), \quad j = 1,\dots,m. $$ This function $g$ is exactly where the non-linearity of the model is introduced: if $g$ were the identity, the hidden units would just be linear functions of $X$ and the overall model would remain linear. By choosing a non-linear $g$ (such as a sigmoid or ReLU, discussed later), we obtain transformed features $Z_1,\dots,Z_m$ that can capture complex relationships in the data. Importantly, the number $m$ of hidden units does not need to equal $p$; it is a modelling choice that controls how rich this intermediate representation is.
...
![Sigmoid activation (left) and ReLU activation (right) as typical choices for the hidden layer non-linearity.](Images_qmd_ML/ML10_3.png){#fig-activation-plot}
...
where $Z \in \mathbb{R}^m$ is the vector of hidden activations. In the output layer we now use a **weight matrix** $W^{(2)} \in \mathbb{R}^{q \times m}$ and a **bias vector** $b^{(2)} \in \mathbb{R}^q$. For each class $i = 1,\dots,q$, the network computes
$$ \hat{f}_i(X) = h_i\big(b^{(2)}_i + w^{(2)\top}_i Z\big), $$
where $w^{(2)}_i$ is the $i$-th row of $W^{(2)}$ and $h_i$ is an output function that will be chosen to turn this linear combination into a valid probability (softmax will be introduced next).

The architecture in @fig-class-hlnn visualises this: the green nodes represent the original predictors, the blue nodes are the shared hidden layer $Z_1,\dots,Z_m$, and the red output nodes $Y_1,\dots,Y_q$ compute the $q$ class probabilities by applying their own functions $h_i$ to different linear combinations of the same hidden activations.
...
For classification we keep the same hidden layer as before, typically with ReLU activation $g(x) = \max\{0,x\}$. The difference lies in the output layer: instead of a single output, we now want $q$ class probabilities. The output functions $h_i$ are chosen to be the **softmax** functions. Given $q$ input scores $x_1,\dots,x_q$, the $i$-th softmax component is $$ h_i(x_1,\dots,x_q) = \text{softmax}(x_1,\dots,x_q)_i = \frac{\exp(x_i)}{\sum_{j=1}^q \exp(x_j)}, \quad i = 1,\dots,q. $$
...
The architecture in @fig-class-hlnn implements exactly this computation: the hidden layer builds non-linear features with ReLU, and the $q$ output nodes apply softmax to produce class probabilities.
