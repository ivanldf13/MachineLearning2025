...
2.  For iterations $i=1,2,\dots$:
    ...
    2.  Update the parameters with learning rate $\alpha>0$,[^3] $$\theta^{(i)}=\theta^{(i-1)}-\alpha\nabla_\theta J(\theta^{(i-1)}).$$
...
[^3]: $\alpha$ is the learning rate (step size) in gradient descent: it controls how far we move in the direction of the negative gradient at each update. A large $\alpha$ gives big, potentially unstable steps, while a small $\alpha$ yields slow but more stable progress.
